...
def simulate_subjects(param_ranges, tasks, fixed_params=None):
    """
    ...
    Parameters:
        ...
        tasks (list)        - List of task lists, tasks[i] contains tasks for subject i
        fixed_params (dict) - Parameters held constant across subjects

    Returns:
        subjects (list)     - List of Subject instances with simulated responses
    ...
    # Simulate each subject on their tasks
    subjects = []
    for s in range(n_subjects):
        ...
        # Create subject and simulate on first task (for parameter recovery, one task per subject)
        subj = Subject()
        task = tasks[s][0] if isinstance(tasks[s], list) else tasks[s]

        # Use task noise_sd and hazard unless we're fitting those
        if 'noise_sd' not in param_ranges:
            subj_params['noise_sd'] = task.noise_sd[0] if not np.isscalar(task.noise_sd) else task.noise_sd
        if 'hazard' not in param_ranges:
            subj_params['hazard'] = task.hazard

        # Simulate subject behavior
        subj = simulate_subject(subj, task.obs, subj_params)
...
def get_negloglike(subj, task, opt_param_names, opt_param_vals, fixed_params=None):
    ...
    # Use task noise_sd and hazard unless optimizing them
    if 'noise_sd' not in opt_param_names:
        noise_sd = task.noise_sd if np.isscalar(task.noise_sd) else task.noise_sd[0]
        params['noise_sd'] = noise_sd
    if 'hazard' not in opt_param_names:
        params['hazard'] = task.hazard

    # Infer beliefs under these parameters
    noise_sd_vec = task.noise_sd if not np.isscalar(task.noise_sd) else np.full(len(task.obs), task.noise_sd)
    ...
    update_sd_int, update_sd_slope = 1.0, 0.1
...
def fit_parameters(subjects, tasks, opt_param_names, n_reps=5, n_processes=None, fixed_params=None):
    """
    Fit parameters for multiple subjects with multiprocessing.

    Parameters:
        subjects (list)       - List of Subject instances
        tasks (list)          - List of task lists, tasks[s][t] is task t for subject s
        opt_param_names (list)- Parameter names to optimize
        n_reps (int)          - Number of fitting repetitions per subject-task
        n_processes (int)     - Number of parallel processes
        fixed_params (dict)   - Parameters held fixed during optimization

    Returns:
        results (dict) with:
            'ests' (ndarray) - Shape [n_subj, n_tasks, n_reps, n_params]
            'nlls' (ndarray) - Shape [n_subj, n_tasks, n_reps]
            'param_names' (list)
    """
    n_subj = len(subjects)
    n_params = len(opt_param_names)

    # Normalize tasks to list-of-lists format
    if not isinstance(tasks[0], list):
        tasks = [[t] for t in tasks]
    n_tasks = len(tasks[0])

    # Pre-allocate results arrays
    ests = np.full((n_subj, n_tasks, n_reps, n_params), np.nan)
    nlls = np.full((n_subj, n_tasks, n_reps), np.nan)
    ...
    for s in range(n_subj):
        for t in range(n_tasks):
            for r in range(n_reps):
                ...
                jobs.append((deepcopy(subjects[s]), deepcopy(tasks[s][t]), seed))
                job_map.append((s, t, r))
    ...
    # Collect results
    n_success = 0
    for result, (s, t, r) in zip(results_list, job_map):
        if result['success']:
            ests[s, t, r, :] = result['estimates']
            nlls[s, t, r] = result['nll']
            n_success += 1
        else:
            print(f"Fit failed for subject {s}, task {t}, rep {r}: {result['error']}")
...
def recovery_stats(results):
    ...
    Returns:
        ...
        var_rep (ndarray)  - Variance over repetitions (averaged over tasks), shape [n_subj, n_params]
        var_task (ndarray) - Variance over tasks (of mean over reps), shape [n_subj, n_params]
        std_tot (ndarray)  - Total standard deviation, shape [n_subj, n_params]
    """
    ests = results['ests']  # Shape: [n_subj, n_tasks, n_reps, n_params]

    # Mean parameter estimates (over tasks and reps)
    mean = np.nanmean(ests, axis=(1, 2))

    # Variance over repetitions for same task, averaged over tasks
    var_rep = np.nanmean(np.nanvar(ests, axis=2), axis=1)

    # Variance over tasks of the mean over repetitions
    mean_over_reps = np.nanmean(ests, axis=2)
    var_task = np.nanvar(mean_over_reps, axis=1)

    # Total standard deviation
    std_tot = np.sqrt(var_rep + var_task)

    return mean, var_rep, var_task, std_tot
...
def analyze_recovery(true_params, results):
    ...
    mean_ests, var_rep, var_task, std_tot = recovery_stats(results)
...
def parameter_recovery(param_names, n_subjects=20, n_tasks_per_subject=5, n_reps=5,
    ...
    Parameters:
        ...
        n_reps (int)               - Fitting repetitions per subject-task
    ...
    subjects, true_params = simulate_subjects(param_ranges, tasks, fixed_params)

    # Fit parameters
    print(f"Fitting parameters: {param_names}")
    results = fit_parameters(subjects, tasks, param_names, n_reps, n_processes, fixed_params)
    ...
    for param_name in param_names:
        ...
        print(f"  {param_name}: r={a['correlation']:.3f}, bias={a['mean_bias']:.3f}, "
              f"std_rep={np.sqrt(a['var_rep']):.3f}, std_task={np.sqrt(a['var_task']):.3f}")
